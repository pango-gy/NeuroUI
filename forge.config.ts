...
const apkName = 'Neuro_' + packageJson.version + '_' + (process.env.arch || process.arch);
...
module.exports = {
  packagerConfig: {
    asar: {
      unpack: '**/node_modules/{node-pty,bcrypt,better-sqlite3,@mapbox,detect-libc,prebuild-install,node-gyp-build,bindings,web-tree-sitter,tree-sitter-bash,@negokaz/excel-mcp-server}/**/*',
    }, // Enable asar with native modules and their dependencies unpacking
    executableName: 'Neuro',
    out: path.resolve(__dirname, 'out'),
    tmpdir: path.resolve(__dirname, '../Neuro-tmp'),
    extraResource: [path.resolve(__dirname, 'public')],
    win32metadata: {
      CompanyName: 'Pango',
      FileDescription: 'AI Agent Desktop Interface',
      OriginalFilename: 'Neuro.exe',
      ProductName: 'Neuro',
      InternalName: 'Neuro',
